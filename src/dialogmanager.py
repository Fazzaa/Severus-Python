...
class DialogManager:
    ...
    def get_matched_patterns(self, name_pattern, pattern, text):
        ...
        matched_elements = [(self.nlp.vocab[matches[0][0]].text, doc[match[1]:match[2]][1:]) for match in matches] 
        ...
        for match in matches:
            if match != []:
                ...
                if match_words[0] > match_words[1]: #! A volte vengono restituiti due indici, a volte 3, bisogna capire come restituire le parole in maniera decente
                    matched_elements.append((self.nlp.vocab[matches[0][0]].text, doc[match_words[2]:match_words[0]][:2]))
                else:
                    matched_elements.append((self.nlp.vocab[matches[0][0]].text, doc[match_words[0]:match_words[1]+1][1:]))#? doc[match_words[0] : match_words[1]+1] se non metto il +1 si mangia l'ultima parola ??
